...
#pragma endregion
    ...
    */
    ...
    # 클래스 사용 범위
    ***
    클래스 사용 범위는 클래스 데이터 멤버들의 이름이나 클래스 멤버 함수들의 이름과 같이,
    클래스 안에서 정의되는 이름들에 적용된다. 클래스 사용 범위를 갖는 것들은 클래스 안에는 알려지지만
    클래스 바깥에는 알려지지 않는다. 그래서 클래스 멤버들을 서로 다른 클래스에 같은 이름으로 선언해도 충돌하지 않는다.
    예를 들어, Stock 클래스의 shares 멤버는, JobRide 클래스의 shares 멤버와 완전히 다른 변수이다.
    또한, 클래스 사용 범위는 사용자가 바깥 세계에서 클래스 멤버들에 직접 접근할 수 없다는 것을 의미한다.
    이것은 public 함수 멤버에도 해당되는 것이다. 예를 들어 public 멤버 함수를 호출하려면 객체를 사용해야 한다.

    ```cpp
        Stock sleeper("Exclusive Ore", 100, 0.25);  // 객체를 생성한다.
        sleeper.show();                             // 객체를 사용하여 멤버 함수를 호출한다.
        show();                                     // 틀리다 -- 메서드는 직접 호출할 수 없다.
    ```

    이와 비슷하게, 멤버 함수들을 정의할 때에는 사용 범위 결정 연산자를 사용해야 한다.
    ...
    간단히 말하면 클래스 선언이나 멤버 함수 정의 안에서 사용자는 약식 ( unqualified ) 멤버 이름을 사용할 수있다.
    그 밖의 경우에 클래스 멤버 이름을 사용할 때에는, 상황에 따라 직접 멤버 연산자 (.)나 간접 멤버 연산자 ( -> ) 또는
    사용 범위 결정 연산자 ( :: ) 를 사용해야 한다.
    ***
    출처 : C++ 기초 플러스 6판 / 성안당
    ***
    */
    #pragma endregion
    #pragma region 10.클래스 사용 범위 상수
    /*
    ---------------------------------------- 10.클래스 사용 범위 상수 ----------------------------------------
    # 클래스 사용 범위 상수
    ***
    클래스 사용 범위를 가진 기호 상수를 사용하는 것이 좋을 때가 있다.
    하지만 다음과 같이 잘못 생각하는 경우 또한 있다.
    ...
    이것은 동작하지 않는다. 클래스를 선언하는 것은 객체가 어떻게 생겼는지 서술하는 것이지,
    그 객체를 생성하는 것은 아니기 때문이다. 따라서 값을 저장할 기억 공간은 객체가 생성될 때까지 마련되지 않는다.

    하지만 원하는 것과 동일한 효과를 내는 두 가지 방법이 있다.

    첫째, 클래스 안에 열거체를 선언할 수 있다. 클래스 선언 안에 주어지는 열거체는 클래스 사용 범위를 가진다.
    ...
    둘째, static 키워드를 사용하여 클래스 안에 상수를 정의하는 방법이다.
    ...
    */
    #pragma endregion
    #pragma region 10.범위가 정해진 열거
    /*
    ---------------------------------------- 10.범위가 정해진 열거 ----------------------------------------
    # 범위가 정해진 열거
    ***
    전통적인 열거는 몇 가지 문제점이 있다.
    ...
    이 구문은 egg와 t_shirt 모두 동일한 범위에 있어 서로 충돌이 발생한다.

    이를 위한 해결책으로 C++11 부터는 열거자에게 클래스 범위를 갖게 함으로써 이 문제를 해결한다.
    ...
    ***
    일반적인 열거는 int 변수에 대한 대입이나 비교 표현에 사용되는 것처럼
    일부 상황에서는 자동적으로 int형으로 전환되지만, 범위가 지정된 열거는 int형으로의 암시적 전환이 이루어지지 않는다.
    그러나, 꼭 필요하다면 명시적 형 변환은 가능하다.

    ```cpp
        enum class egg  { Small, Medium, Large, Jumbo };
        
        int x = egg::Small;         // 허용되지 않음
        int y = int(egg::Small);    // y는 0에 세팅된다
    ```

    ***
     
    다음과 같은 방식으로 열거의 내재적 형태인 int가 아닌, 다른 선택을 의미하는 구문이 있다.
    
    ```cpp
        enum class : short pizza { Small, Medium, Large, Jumbo };
    ```

    ' : short ' 는 기본형이 short가 되어야 함을 의미한다.
    기본 변수형은 정수형이 되어야만 한다. 사용자가 이런 식으로 형태를 선택하지 않으면
    컴파일러가 실행하는 방식에 따라 임의로 선택을 하게 된다.
    ***
    출처 : C++ 기초 플러스 6판 / 성안당
    ***
    */
    #pragma endregion
    #pragma region 10.추상화 데이터형
    /*
    ---------------------------------------- 10.추상화 데이터형 ----------------------------------------
    # 추상화 데이터형
    ***
    **추상화 데이터형 ( Abstract Data Type, ADT )**는 언어나 시스템의 세부사항을 따지지 않고,
    데이터형을 일반적인 형식으로 서술한다. 한 예로 스택을 생각해 보자.
    스택을 사용함으로써, 데이터의 추가나 삭제가 꼭대기에서 이루어지도록 데이터를 저장할 수 있다.
    가령 C++ 프로그램은, 스택을 사용하여 자동 변수를 관리한다.

    이제 스택의 특성을 일반적이고 추상적인 방식으로 살펴보자.
    스택은 여러 개의 항목을 저장한다. 그 다음, 다음과 같은 연산을 수행할 수 있는 특성을 지닌다.

        * 빈 스택을 생성할 수 있다.
        * 스택의 꼭대기에 항목을 추가할 수 있다. ( = push )
        * 스택의 꼭대기에서 항목을 삭제할 수 있다. ( = pop )
        * 스택이 가득 차 있는지 검사할 수 있다.
        * 스택이 비어 있는지 검사할 수 있다.
    
    public 멤버 함수들로 스택 연산을 나타내는 인터페이스를 제공하는 클래스 선언과 이 서술을 대응시킬 수 있다.
    스텍 데이터의 저장은 private 데이터 멤버들이 담당한다.

    private 부분은 데이터 저장 방법을 정해야 한다. 예를 들어, 일반 배열, 동적 대입 배열,
    링크드 리스트 ( Linked List ), 또는 좀 더 높은 수준의 자료 구조를 사용할 수 있다.

    그러나 public 인터페이스는 정확한 표현이 드러나지 않게 해야 한다. 
    대신 스택을 생성한다, 항목을 푸시한다는 등의 일반적인 용어로 표현해야 한다.
    ***
    출처 : C++ 기초 플러스 6판 / 성안당
    ***
    */
...
#pragma region Chapter 12. 클래스와 동적 메모리 대입

    #pragma region 12.Intro
    /*
    ---------------------------------------- 12.Intro ----------------------------------------
    # Intro
    ***
    이 장에서는 new와 delete를 클래스와 함께 사용하는 방법과, 동적 메모리를 사용할 때,
    일어날 수 있는 몇 가지 미묘한 문제들을 처리하는 방법을 알아본다.

    기억 공간을 얼마나 사용하느냐와 같은 중요한 문제들을, 프로그램을 컴파일할 때가 아니라
    실행할 때 결정하도록 만드는 것이 훨씬 좋다. 객체에 이름을 저장하기 위해 C++가 사용하는
    일반적인 방법은, 클래스 생성자에 new 연산자를 사용하여 프로그램을 실행할 때 그 이름에 꼭 맞는
    크기의 메모리를 대입하는 것이다. 클래스 생성자에 new를 도입하기 위해서는 몇 가지
    추가적인 일련의 단계가 필요하다. 예를 들면, 클래스 파괴자를 확장하고 모든 생성자를 그 new 파괴자와
    어울리게 수정해야하며, 초기화와 대입을 사용할 수 있도록 부가적인 클래스 메서드를 작성하는 등
    일련의 절차를 밟아야 한다.
    ***
    출처 : C++ 기초 플러스 6판 / 성안당
    ***
