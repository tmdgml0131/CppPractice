...
#pragma region Chapter 01
    ...
    ***
    ...
    ```
    첫 번재 foo(5) 호출에서 인수는 리터럴 5입니다. foo(5)가 호출되면 변수 y가 만들어지고
    ...
    main 함수의 시작 부분에서 변수 x의 값은 5입니다. foo(X)가 호출되면 foo의 매개변수 y에 값 5가 복사되어 전달됩니다.
    foo() 함수 내부에서 y의 값은 6으로 변경되고 소멸하지만, y가 변경되더라도 변수 x의 값은 아무런 영향을 받지 않습니다.
    ...
    * 큰 구조체 또는 클래스를 함수에 전달할 때 값으로 전달은 인수의 복사본을 함수 매개 변수로 만든다.
      이 경우, 복사하는데 큰 비용이 들어 성능이 저하될 수 있다.
    * 값으로 인수를 전달할 경우, 함수에서 호출자에게 값을 반환하는 유일한 방법은 함수의 반환 값을 사용하는 것이다.
      이 방법도 좋지만, 함수에서 인수를 수정하는 것이 더 명확하고 효율적일 수 있다.

    변수를 참조로 전달하려면 함수 매개 변수를 일반 변수가 아닌 참조로 선언해야 합니다.
    ```cpp
        void addOne(int& y)
        {
            y = y+1;
    ...
    함수가 호출되면 y는 인수에 대한 참조가 된다. 변수에 대한 참조는 변수 자체와 똑같이 취급되므로
    ...
    **const 참조는 변수가 참조를 통해 변경되는 것을 허용하지 않는 참조**입니다.
    그러므로 const 참조를 매개변수로 사용하면 함수가 인수를 변경하지 않는다는 것을 호출자에게 보장합니다.
    ...
    ***
    ...
    포인터를 참조로 전달하고, 함수가 포인터의 주소를 완전히 변경하도록 할 수 있습니다.
	#include <iostream>
    ...
    * 참조는 초기화되어야 하므로 null 값에 대해 걱정할 필요가 없다.
    ...
    ### 참조로 전달의 단점
    * non-const 참조는 const 값, 또는 r-value로 초기화할 수 없으므로 참조 매개변수에 대한 인수는 일반 변수여야 한다.
    * 인수가 변경될 수 있는지는 함수 호출에서 알 수 없다. 값으로 전달이나 참조로 전달 모두 인수에서는 동일하게 보인다.
      함수 선언을 보고 인수가 값으로 전달인지 참조로 전달인지 알 수 있다. 이것은 프로그래머가 함수에서 인수의 값을
	...
	*/
    #pragma endregion
    #pragma region 1.9 예외처리
	/*
    ***
    # 개요
    이상적인 상항에선, 프로그램 혹은 라이브러리 사용자들은 언제나 올바른 값을 입력 값으로 줄 것이고,
    컴퓨터 역시 무한한 자원을 사용할 수 있어서 어떠한 상황에서도 데이터들을 정상적으로 처리할 수 있을 것 입니다.
    
    하지만 사람은 실수를 하기 마련이고, 컴퓨터 역시 항상 프로그램에 필요한 자원을 제공할 수 있는 것이 아닙니다.
    ...
    ```cpp
        ...
        cout << v.at(4);        // ??
    ```
    위 코드에서는 문법상 아무런 문제도 없지만 막상 실행하면 오류가 발생합니다.

    이렇게 정상적인 상황에서 벗어난 모든 예외적인 상황들을 **예외 (Exception)** 이라고 부릅니다.
    ***
    # 기존의 예외 처리 방식
    C언어에서는 언어 차원에서 제공하는 예외 처리 방식이라는 것이 딱히 따로 존재하지 않았습니다.
    따라서 아래와 같이, 어떤 작업을 실행한 후, 그 결과값을 확인하는 방식으로 처리하였습니다.
    ```cpp
    ...
    ```
    `malloc`의 경우 메모리 할당 실패 시에 NULL을 리턴하므로, 위와 같이 `c`가 `NULL`인지 확인함으로써 예외처리가 가능합니다.

    하지만 이런 방식은 함수가 깊어질수록 매우 귀찮아집니다.

    만약 `func3`라는 함수에 '예외가 발생할 수 있는 작업' 을 수행하는데 이 `func3` 함수가
    `func2` 함수에서, `func2` 함수는 `func1` 함수에서, `func1`함수는 `main`에서 호출된다면
    예외를 잡기 위해, 각각의 함수에서 처리 결과를 모두 리턴해야합니다.

    예외가 `func3`에서만 발생하면 간단하겠지만, 만약 `func2`도 예외를 발생시키면
    이는 상당히 골치 아파집니다.
	...
	*/
#pragma endregion
    #pragma region 1.12 에러와 인터프리터, 그리고 컴파일러
	/*
    ***
    # 개요
    여러분들이 아무리 실력이 뛰어난 프로그래머라 하더라도 프로그래밍을 하면서
    한번 쯤은 에러를 만나게 됩니다. 하지만 이 에러에도 여러 종류가 있다는 사실
    알고 계셨나요??
    ***
	...
	***
	# 파스 에러 ( Parse Error )
	인터프리터 언어에서 문법의 오류로 인해 나타나는 에러 ( = 인터프리터의 컴파일 에러 )
	***
    # 컴파일러 vs 인터프리터
    컴파일러와 인터프리터는 고급언어로 작성된 원시 프로그램 ( Source Program ) 을 
    목적 프로그램 ( Object Program ) 으로 번역하는 번역 프로그램이며, 번역 방식에 따라 구분 됩니다.
    ***
    ## 컴파일러
    * 컴파일러는 고급 언어로 작성된 프로그램 전체를 목적 프로그램으로 번역한 후, 링킹 작업을 통해
      컴퓨터에서 실행 가능한 실행 프로그램을 생성합니다.
    * 번역 실행 과정을 거쳐야 하기 때문에 번역 과정이 번거롭고 번역 시간이 오래 걸리지만, 한번 번역한 후에는
      다시 번역하지 않으므로 실행 속도가 빠릅니다.
    * 컴파일러를 사용하는 언어에는 C언어 Java 등이 있습니다.
    ***
    ## 인터프리터
    * 인터프리터는 고급 언어로 작성된 프로그램을 한 줄 단위로 받아들여 번역하고, 번역과 동시에
      프로그램을 한 줄 단위로 즉시 실행시키는 프로그램입니다.
    ...
    * 인터프리터를 사용하는 언어에는 Python, BASIC, SNOBOL, APL 등이 있습니다.
    * CPU의 사용시간의 낭비가 큽니다.
...
#pragma endregion
#pragma region Chapter 02

    #pragma region 2.1 스트링
	/*
    ***
    # 개요
    C에서는 `char*` 또는 `char[]`의 형태로 문자열을 다뤘다면,
    C++에서는 문자열을 하나의 변수 Type 처럼 사용합니다. 문자열을 변수 type으로 사용하며,
    문자열을 훨씬 다양하고 쉽게 다룰 수 있게 해주는데요, `char*`, `char[]`과 다르게
    **문자열의 끝에 `\0` 문자가 들어가지 않으며, 문자열의 길이를 동적으로 변경 가능합니다!
    **string** 은 C++ STL에서 제공하는 클래스로, 말 그대로 문자열을 다루는 클래스입니다.
    ***
    # String 클래스의 입/출력
    C++의 입출력 방식인 `cin`, `cout`으로 입출력이 가능하며, `getline` 함수도 사용할 수 있습니다.
    ```cpp
        string str;             // 문자열 생성
    ...
    ***
    ...
    str.resize(n);
    str을 n의 크기로 만든다. 기존보다 n이 작다면 남은 부분 삭제, 크다면 빈 공간으로 채움
